...
function parseHtmlBlogFile(htmlContent) {
  ...
  return {
    data: {
      ...
      og_type:     "article",
      og_image:    "https://kieleckinotariusz.pl/assets/images/library-bg.jpg",
      schema_json,
    },
...
function compileTemplate(layout, partials, pageData, relativeRoot, activeLinkName) {
  ...
  html = html
    ...
    .replaceAll("{{og_image}}",    pageData.og_image    || `${relativeRoot}assets/images/library-bg.jpg`)
    .replaceAll("{{schema_json}}", pageData.schema_json || "");

  let headerHtml = partials.header.replaceAll("{{relativeRoot}}", relativeRoot);
  if (activeLinkName) {
    const targetHref = `href="${relativeRoot}${activeLinkName}"`;
...
async function main() {
  ...
  // Mapa sekcji Hub & Spoke: id kotwicy → { tytuł, wstęp z linkami, kategorie }.
  // Każda sekcja otrzymuje semantyczne <section id="..."> wykorzystywane przez
  // linki zwrotne z artykułów blogowych (np. dokumenty.html#malzenstwo).
  const DOC_SECTIONS = [
    {
      id: "malzenstwo",
      title: "Małżeństwo i majątek",
      intro: `Sprawy małżeńskie i majątkowe – od umowy majątkowej małżeńskiej po podział majątku wspólnego. Więcej w poradnikach: <a href="{{relativeRoot}}blog/majatek-wspolny-osobisty/">majątek wspólny a majątek osobisty małżonków</a> oraz <a href="{{relativeRoot}}blog/rozdzielnosc-majatkowa/">rozdzielność majątkowa małżeńska</a>.`,
      folders: ["05-podzial-majatku-wspolnego", "10-umowa-majatkowa-malzeska"],
    },
    {
      id: "nieruchomosci",
      title: "Nieruchomości",
      intro: `Obrót nieruchomościami i ich obciążenia: sprzedaż, zamiana, darowizna, umowa o dożywocie, hipoteka, służebności, najem okazjonalny oraz odrębna własność lokalu. Zobacz poradnik: <a href="{{relativeRoot}}blog/testament-darowizna-dozywocie/">darowizna, testament czy umowa o dożywocie – jak przekazać nieruchomość</a>.`,
      folders: ["01-umowa-sprzedazy", "02-umowa-zamiany", "03-umowa-darowizny", "06-umowa-o-dozywocie", "08-pelnomocnictwo-do-zbycia-nieruchomosci", "11-ustanowienie-hipoteki", "13-sluzebnosc-gruntowa", "14-sluzebnosc-osobista", "15-rygor-egzekucji-z-najmu-okazjonalnego", "16-rygor-egzekucji", "18-ustanowienie-odrebnej-wlasnosci-lokalu"],
    },
    {
      id: "spadki",
      title: "Spadki",
      intro: `Sprawy spadkowe: dział spadku i zniesienie współwłasności, testament, akt poświadczenia dziedziczenia oraz przyjęcie lub odrzucenie spadku. Więcej w poradniku: <a href="{{relativeRoot}}blog/testament-darowizna-dozywocie/">testament a darowizna i umowa o dożywocie</a>.`,
      folders: ["04-dzial-spadku-zniesienie-wspolwlasnosci", "07-testament", "09-akt-poswiadczenia-dziedziczenia", "12-odrzucenie-lub-przyjecie-spadku"],
    },
    {
      id: "spolki",
      title: "Firmy i fundacje",
      intro: `Sprawy związane z firmą rodzinną i sukcesją majątku – m.in. fundacja rodzinna jako narzędzie zarządzania majątkiem i przekazywania go kolejnym pokoleniom.`,
      folders: ["17-fundacja-rodzinna"],
    },
  ];

  const categoryHtmlByFolder = {};
  ...
  for (const folder of docFolders) {
    ...
    categoryHtmlByFolder[folder.name] = categoryHtml;
  }

  // Złożenie akordeonów w semantyczne sekcje z unikalnymi id (kotwice Hub & Spoke)
  const assignedFolders = new Set();
  let accordionListHtml = "";

  for (const section of DOC_SECTIONS) {
    const blocks = section.folders
      .filter(name => {
        if (categoryHtmlByFolder[name]) { assignedFolders.add(name); return true; }
        return false;
      })
      .map(name => categoryHtmlByFolder[name])
      .join("\n");
    if (!blocks) continue;
    ...
    accordionListHtml += `
      <section id="${section.id}" class="docs-section">
        ...
        <p class="docs-section__intro">${section.intro}</p>
        <div class="accordion-list">${blocks}
        </div>
      </section>\n`;
  }

  // Awaryjnie: kategorie nieprzypisane do żadnej sekcji trafiają do "Pozostałe"
  const leftover = docFolders
    .map(f => f.name)
    .filter(name => categoryHtmlByFolder[name] && !assignedFolders.has(name))
    .map(name => categoryHtmlByFolder[name])
    .join("\n");
  if (leftover) {
    accordionListHtml += `
      <section id="pozostale" class="docs-section">
        <h2 class="docs-section__title">Pozostałe czynności</h2>
        <div class="accordion-list">${leftover}
        </div>
      </section>\n`;
  }
  ...
  // 4. Artykuły blogowe
  console.log("✍️  Kompilacja bloga...");
  const blogSrcDir = path.join(SITE_SRC, "blog");
  if (existsSync(blogSrcDir)) {
    for (const blogFilePath of await walk(blogSrcDir)) {
      if (!blogFilePath.endsWith(".html")) continue;
      const rel = path.relative(blogSrcDir, blogFilePath);
      console.log(`   - blog/${rel.replace(/\\/g, "/")}`);
      const outPath = path.join(PUBLIC, "blog", rel);
      await fs.mkdir(path.dirname(outPath), { recursive: true });
      const rawBlog = await fs.readFile(blogFilePath, "utf8");
      const { data, content } = parseHtmlBlogFile(rawBlog);
      await fs.writeFile(outPath, compileTemplate(layout, partials, { ...data, content }, "../../", "blog.html"), "utf8");
    }
  }
