 ...
 *
 *   /          → public/          (wygenerowany HTML/CSS/JS)
 *   /assets/   → assets/          (obrazy, czcionki, PDF-y)
 *
...
function resolve(urlPath) {
  if (urlPath === "/favicon.ico") return path.join(ASSETS, "favicon.ico");
  if (urlPath.startsWith("/assets/")) return path.join(ASSETS, urlPath.slice("/assets/".length));
  return path.join(PUBLIC, urlPath);
}

function isAllowed(filePath) {
  return filePath.startsWith(PUBLIC) || filePath.startsWith(ASSETS);
}
...
server.listen(PORT, () => {
  ...
  console.log(`   /          → public/`);
  console.log(`   /assets/   → assets/  (PDFy: X-Robots-Tag: noindex, nofollow)`);
